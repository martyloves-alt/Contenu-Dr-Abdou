"""Montage v5 : v4 + voix additionnelles d'Abdou (notes vocales), coupe d'hésitation,
...
TMP = os.path.join(OUT, "tmp_montage_v5")
...
SECTIONS = [
    ...
    dict(nom="Annonce piège 3", audio=[("210605", 0.00, 3.75), ("WA0039", 1.80, 3.30)], video=[
        ("205853", 0.25, 1.45, None),            # main ouverte tendue : badge dans la paume
        ("210605", 1.20, 3.75, None),
        ("204909", 2.40, 3.90, "punch"),         # voix off « Trois pièges à connaître » (Abdou bouche fermée)
    ]),
    ...
    dict(nom="Piège 2", audio=[("WA0040", 1.20, 2.30), ("210713", 0.15, 4.20), ("210713", 5.60, 8.45), ("210713", 10.05, 11.00)], video=[
        ("204909", 5.70, 6.80, None),            # voix off « Piège deux »
    ...
    dict(nom="TDR", audio=[("WA0042", 1.80, 3.30), ("205654", 1.40, 5.15)], video=[
        ("204909", 9.00, 10.50, None),           # voix off « Un seul test le dit »
    ...
    dict(nom="Action", audio=[("210140", 0.00, 3.00), ("WA0041", 0.70, 2.17), ("WA0041", 2.35, 2.97), ("WA0041", 3.21, 4.19), ("WA0041", 4.33, 5.17), ("WA0041", 5.75, 6.53), ("WA0041", 7.09, 8.23), ("WA0041", 8.45, 9.97), ("SIL", 0.00, 0.75)], video=[
        ("204909", 0.95, 1.85, None),            # balayage 1 → ligne 1
        ("210140", 0.90, 3.00, None),
        ("204909", 3.45, 6.45, None),            # voix off « …convulse… » : balayage 2 → ligne 2
...
SUBS = [
    ...
    (1, 3.75, 5.25, "Trois pièges à connaître."),
    (2, 0.00, 2.05, "Piège 1 : croire aux dents."),
    (2, 2.05, 4.90, "Les dents ne donnent pas de forte fièvre."),
    (3, 0.00, 1.10, "Piège 2 : attendre."),
    (3, 1.10, 5.15, "Au Bénin, un enfant fiévreux, on cherche d'abord le palu."),
    (3, 5.15, 8.95, "Sans traitement, en 24 heures, il peut devenir grave."),
    (4, 0.00, 2.50, "Piège 3 : le paracétamol."),
    (4, 2.50, 5.30, "Il fait baisser la fièvre."),
    (4, 5.30, 7.70, "Il ne soigne pas le palu."),
    (5, 0.00, 2.60, "Un seul test le dit : le TDR."),
    (5, 2.60, 3.90, "Une goutte de sang."),
    (5, 3.90, 5.25, "15 à 20 minutes."),
...
CHECK_T = [(0.25, 0.85), (4.10, 4.75), (7.42, 8.00)]  # révélation de chaque ligne (section 6)
...
def build_overlay(path):
    ...
    trois = stack(label("3", 190, WHITE, RED, pad=40, radius=40), label("PIÈGES", 96, WHITE, DARK_T))
...
def main():
    ...
    dst = a.sortie or os.path.join(OUT, "ep01_montage_v5.mp4")
    import sfx
    S = SPEED
    cues = dict(A=at(0, 3.54) / S, B=at(1, 3.41) / S, D=at(3, 6.48) / S, E=at(5, 0.48) / S,
                F=at(6, 0.85) / S, G=at(6, 10.35) / S)
