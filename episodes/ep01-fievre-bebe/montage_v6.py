"""Montage v6 : v5 + bannières une à une, zoom lent, Piège 3 resynchronisé, notes vocales ×1,2.
Historique : v5 = v4 + voix additionnelles d'Abdou (notes vocales), coupe d'hésitation,
habillage sonore (sfx.py) et mots « Piège » / « Paracétamol » en rouge dans les sous-titres.

Usage : python3 montage_v5.py → out/ep01_montage_v5.mp4
Notes vocales attendues dans assets/voix/WA00xx.wav (normalisées à -24 LUFS, voir rapport).
...
TMP = os.path.join(OUT, "tmp_montage_v6")
W, H, FPS = 1080, 1920, 30
SPEED = 1.18  # accélération globale (voix à hauteur conservée) — resserre le rythme sans risquer de couper un mot
...
VOIX = os.path.join(HERE, "assets", "voix")


def rush(stamp):
    if stamp.startswith("WA"):  # notes vocales ajoutées par Abdou (audio seul)
        return os.path.join(VOIX, f"{stamp}.wav")
...
SECTIONS = [
    ...
    dict(nom="Annonce piège 3", audio=[("210605", 0.00, 3.75), ("WA0039f", 1.5, 2.75)], video=[
        ("205853", 0.25, 1.45, None),            # main ouverte tendue : badge dans la paume
        ("210605", 1.20, 3.75, None),
        ("204909", 2.40, 3.65, "punch"),         # voix off « Trois pièges à connaître » (Abdou bouche fermée)
    ]),
    ...
    dict(nom="Piège 2", audio=[("WA0040f", 1.0, 1.917), ("210713", 0.15, 4.20), ("210713", 5.60, 8.45), ("210713", 10.05, 11.00)], video=[
        ("204909", 5.70, 6.617, None),            # voix off « Piège deux »
        ("210713", 0.15, 4.20, None),
        ("210713", 5.60, 8.45, "punch"),         # coupes de silence masquées par un zoom
        ("210713", 10.05, 11.00, None),
    ]),
    dict(nom="Piège 3", audio=[("210521", 0.00, 1.02), ("210521", 2.32, 7.70)], video=[
        ("210521", 0.00, 1.02, None),            # « Piège 3, le… » (pause de 1,3 s coupée)
        ("205559", 1.00, 2.08, None),            # la boîte apparaît sur le mot « paracétamol »
        ("210521", 3.40, 5.30, None),            # « il fait baisser la fièvre »
    ...
    dict(nom="TDR", audio=[("WA0042f", 1.5, 2.75), ("205654", 1.40, 5.15)], video=[
        ("204909", 9.00, 10.25, None),           # voix off « Un seul test le dit »
    ...
    dict(nom="Action", audio=[("210140", 0.00, 3.00), ("WA0041f", 0.583, 1.808), ("WA0041f", 1.958, 2.475), ("WA0041f", 2.675, 3.492), ("WA0041f", 3.608, 4.308), ("WA0041f", 4.792, 5.442), ("WA0041f", 5.908, 6.858), ("WA0041f", 7.042, 8.308), ("SIL", 0.00, 0.905)], video=[
        ("204909", 0.95, 1.85, None),            # balayage 1 → ligne 1
        ("210140", 0.90, 3.00, None),
        ("204909", 3.55, 6.15, None),            # voix off « …convulse… » : balayage 2 → bannière 2
        ("204909", 6.80, 10.28, "punch"),        # « …à moins de trois mois… » : balayage 3 → bannière 3
...
SUBS = [
    ...
    (1, 3.75, 5.00, "Trois pièges à connaître."),
    (2, 0.00, 2.05, "Piège 1 : croire aux dents."),
    (2, 2.05, 4.90, "Les dents ne donnent pas de forte fièvre."),
    (3, 0.00, 0.92, "Piège 2 : attendre."),
    (3, 0.92, 4.97, "Au Bénin, un enfant fiévreux, on cherche d'abord le palu."),
    (3, 4.97, 8.77, "Sans traitement, en 24 heures, il peut devenir grave."),
    (4, 0.00, 2.10, "Piège 3 : le paracétamol."),
    (4, 2.10, 4.00, "Il fait baisser la fièvre."),
    (4, 4.00, 6.40, "Il ne soigne pas le palu."),
    (5, 0.00, 2.35, "Un seul test le dit : le TDR."),
    (5, 2.35, 3.65, "Une goutte de sang."),
    (5, 3.65, 5.00, "15 à 20 minutes."),
    (6, 0.00, 3.00, "Fièvre ? Test au centre de santé dans les 24 heures."),
    (6, 3.00, 6.70, "Convulsions, ne tète plus, vomit tout, dort trop ?"),
    (6, 6.70, 10.03, "Moins de 3 mois ? Urgences."),
...
CHECK_T = [(0.25, 0.85), (3.95, 4.55), (6.70, 7.30)]  # révélation de chaque ligne (section 6)
...
# Vidéo
# ---------------------------------------------------------------------------------------
FACE = {"204549", "210605", "210310", "210713", "210521", "205654", "210140", "205005", "204909"}  # plans face caméra : zoom lent +5 %
...
def render_piece(k, piece):
    ...
    if fx == "colorfix":
        vf.append(COLORFIX)
    vf.append(CROP)
    if sid in FACE and fx in (None, "punch"):
        z0 = 1.1 if fx == "punch" else 1.0
        vf.append(f"scale=2160:3840,zoompan=z='{z0}+0.05*on/{max(1, n - 1)}':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)'"
...
def build_overlay(path):
    ...
    badge2 = label("PIÈGE N°2", 92, WHITE, RED, pad=38, radius=34)
    trois = stack(label("3 PIÈGES", 110, WHITE, RED, pad=40), label("À CONNAÎTRE", 72, WHITE, DARK_T))
    ...
    for i in range(n):
        ...
        if at(1, 3.75) <= t < at(1, 5.00):
            paste_center(L, trois, W / 2, 1000, scale=pop(t, at(1, 3.75)))
        if at(3, 0.00) <= t < at(3, 0.92):
            paste_center(L, badge2, W / 2, 1060, scale=pop(t, at(3, 0.00)))
        # Chrono 24 H (Piège 2), sur « en 24 heures, il peut devenir grave »
        c0, c1 = at(3, 6.32), at(3, 8.77)
        if c0 <= t < c1:
        ...
        # Bannières une à une : chaque balayage fait apparaître la suivante, la précédente s'efface
        for k, (r0, r1) in enumerate(CHECK_T):
            a0, a1 = at(6, r0), at(6, r1)
            end = at(6, CHECK_T[k + 1][0]) if k + 1 < len(CHECK_T) else at(6, 10.03)
            fade = 0.18
            if a0 <= t < end:
                q = min(1, (t - a0) / (a1 - a0))
                row = rows[k]
                ...
                if t > end - fade:
                    al = cut.getchannel("A").point(lambda v, f=(end - t) / fade: round(v * f))
                    cut.putalpha(al)
                L.alpha_composite(cut, (rx, 1000))
...
# ---------------------------------------------------------------------------------------
RED_WORDS = re.compile(r"\b(pièges?|paracétamol)\b", re.IGNORECASE)  # mots graves en rouge
...
"""
    with open(path, "w", encoding="utf-8") as f:
        ...
        for sec, a, b, txt in SUBS:
            style = "SubTop" if sec == 6 or (sec, a) == (4, 4.00) else "Sub"  # checklist / boîte en bas du cadre
...
VOICE_FX = ",".join([  # voix naturelle d'Abdou : nettoyage/clarté seulement, aucun changement de hauteur
    "highpass=f=80",
    "afftdn=nf=-30",
...
def build_audio(path, voice_fx):
    ...
    for k, (sid, a, b) in enumerate(parts):
        if sid == "SIL":
            inputs += ["-f", "lavfi", "-t", f"{b - a:.3f}", "-i", "anullsrc=r=48000:cl=mono"]
        else:
...
def main():
    ...
    dst = a.sortie or os.path.join(OUT, "ep01_montage_v6.mp4")
    import sfx
    S = SPEED
    cues = dict(A=at(0, 3.54) / S, B=at(1, 3.41) / S, D=at(3, 6.30) / S, E=at(5, 0.40) / S,
                F=at(6, 0.85) / S, G=at(6, 9.125) / S)
    print("Repères son (s) :", {k: round(v, 2) for k, v in cues.items()})
    mus, fxp = os.path.join(TMP, "music.wav"), os.path.join(TMP, "fx.wav")
    sfx.build(cues, TOTAL / S + 0.5, mus, fxp)
    fc = (f"[0:v][1:v]overlay=0:0:format=auto,ass='{ass}',setpts=PTS/{SPEED},fps={FPS},format=yuv420p[v];"
          f"[2:a]atempo={SPEED},aformat=sample_rates=48000:channel_layouts=mono,asplit=2[vo][key];"
          f"[3:a][key]sidechaincompress=threshold=0.03:ratio=5:attack=15:release=350[mus];"
          f"[vo][mus][4:a]amix=inputs=3:duration=first:normalize=0,loudnorm=I=-14:TP=-1.5:LRA=11,aformat=channel_layouts=stereo[a]")
    run([FF, "-y", "-i", base, "-i", overlay, "-i", audio, "-i", mus, "-i", fxp, "-filter_complex", fc, "-map", "[v]", "-map", "[a]",
